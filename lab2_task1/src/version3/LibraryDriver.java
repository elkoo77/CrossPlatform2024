...
public class LibraryDriver {
    public static void main(String[] args) {
        ...
        // Створення читачів
        BookReader reader1 = new BookReader("Maria", "Crown", 1, Arrays.asList(book1));
        BookReader reader2 = new BookReader("Stephan", "Smith", 2, Arrays.asList(book2));

        // Створення бібліотеки
        Library library = new Library("City Library", Arrays.asList(bookStore1), Arrays.asList(reader1, reader2));

        // Серіалізація
        try (ObjectOutputStream oos = new ObjectOutputStream(new FileOutputStream("library_externalizable.ser"))) {
        ...
        try (ObjectInputStream ois = new ObjectInputStream(new FileInputStream("library_externalizable.ser"))) {
